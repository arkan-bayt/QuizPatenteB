      ...
      <div className="sticky top-0 z-30 glass-header">
        <div className="max-w-2xl mx-auto px-5 py-4 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <h1 className="text-[15px] font-bold" style={{ color: 'var(--text-primary)' }}>Gestione Utenti</h1>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-5 pt-6 space-y-5">
        ...
        {msg && (
          <div className={`p-4 rounded-2xl text-sm text-center anim-slide-down flex items-center justify-center gap-2 ${msgType === 'success' ? 'feedback-correct' : 'feedback-wrong'}`}>
            {msgType === 'success'
              ? <svg className="w-4 h-4 flex-shrink-0" style={{ color: 'var(--success)' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              : <svg className="w-4 h-4 flex-shrink-0" style={{ color: 'var(--danger)' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" /></svg>
            }
        ...
        <div className="glass p-6 anim-up" style={{ boxShadow: 'var(--shadow-lg)' }}>
          <div className="flex items-center gap-2.5 mb-5">
            ...
            <h2 className="text-[14px] font-bold" style={{ color: 'var(--text-primary)' }}>Aggiungi Utente</h2>
          </div>
          ...
          <div className="flex items-center gap-3 mb-5">
            <span className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>Ruolo:</span>
            ...
            <button onClick={() => setNewRole('user')}
              className="px-4 py-2 rounded-xl text-xs font-semibold transition-all duration-200 border cursor-pointer"
              style={newRole === 'user'
                ? { background: 'var(--accent-150)', color: 'var(--accent)', borderColor: 'var(--accent-200)' }
                : { background: 'transparent', color: 'var(--text-muted)', borderColor: 'var(--border)' }
              }>
              Utente
            </button>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 px-1">
            ...
            <h2 className="text-[12px] font-bold uppercase tracking-[0.12em]" style={{ color: 'var(--text-muted)' }}>Utenti registrati ({users.length})</h2>
          </div>
          ...
          {users.map((u, i) => (
            <div key={u.id} className="glass p-4 flex items-center gap-4 anim-up" style={{ animationDelay: `${i * 40}ms` }}>
              ...
              <div className="w-11 h-11 rounded-2xl flex items-center justify-center flex-shrink-0"
                style={{
                  background: u.role === 'admin' ? 'linear-gradient(135deg, var(--primary-150), var(--primary-100))' : 'linear-gradient(135deg, var(--accent-150), var(--accent-100))',
                  border: `1px solid ${u.role === 'admin' ? 'var(--primary-200)' : 'var(--accent-200)'}`
                }}>
                <span className="text-sm font-bold" style={{ color: u.role === 'admin' ? 'var(--primary-light)' : 'var(--accent)' }}>{u.username[0].toUpperCase()}</span>
              </div>

              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="text-[14px] font-semibold" style={{ color: 'var(--text-primary)' }}>{u.username}</p>
                  {u.username === 'arkan' && (
                    <span className="text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider" style={{ color: 'var(--accent)', background: 'var(--accent-100)', border: '1px solid var(--accent-150)' }}>Super Admin</span>
                  )}
                </div>
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => handleToggleRole(u)}
                  ...
                  style={u.role === 'admin'
                    ? { background: 'var(--primary-100)', color: 'var(--primary-light)', borderColor: 'var(--primary-150)' }
                    : { background: 'var(--accent-100)', color: 'var(--accent)', borderColor: 'var(--accent-150)' }
                  }>
          ...
          {users.length === 0 && (
            <div className="glass p-10 text-center">
              ...
              <p className="text-sm font-medium" style={{ color: 'var(--text-muted)' }}>Nessun utente trovato</p>
              <p className="text-[11px] mt-1" style={{ color: 'var(--text-muted)' }}>Assicurati che la tabella app_users esista in Supabase</p>
            </div>
          )}
        </div>
      </div>
