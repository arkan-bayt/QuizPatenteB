...
import { forceSyncToCloud, saveThemePreference, loadCloudProgress } from '@/logic/progressEngine';
...
export default function HomeScreen() {
  ...
  // Manual sync: download from cloud then upload local changes
  const handleManualSync = async () => {
    if (!username || syncStatus === 'syncing') return;
    setSyncStatus('syncing');
    try {
      // First, upload current local progress to cloud
      await forceSyncToCloud(username);
      // Then, download cloud progress (merge)
      await loadCloudProgress(username);
      // Force re-render to pick up new localStorage data
  ...
  const handleLogout = async () => {
    if (username) await forceSyncToCloud(username);
    clearSession();
    store.setUser(null);
      ...
      <div className="sticky top-0 z-30 glass-header">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 py-3.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5">
            {/* Super_admin / Admin: Dashboard button (gear icon) text = "Admin" → admin panel */}
            {isAdmin && (
              <button onClick={() => store.setScreen('admin')} className="text-xs px-3 py-2 rounded-xl text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-secondary)] transition-colors font-medium flex items-center gap-1.5">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 3v11.25A2.25 2.25 0 006 16.5h2.25M3.75 3h-1.5m1.5 0h16.5m0 0h1.5m-1.5 0v11.25A2.25 2.25 0 0018 16.5h-2.25m-7.5 0h7.5m-7.5 0l-1 3m8.5-3l1 3m0 0l.5 1.5m-.5-1.5h-9.5m0 0l-.5 1.5" /></svg>
                Admin
              </button>
            )}
            {/* Teacher: Dashboard button (gear icon) → teacher dashboard */}
            {isTeacher && (
            ...
            {/* Student: Dashboard button (gear icon) → student dashboard */}
            {isStudent && (
            ...
            {/* Super_admin only: Admin settings button (settings icon) → admin panel */}
            ...
            <button onClick={handleManualSync} disabled={syncStatus === 'syncing'}
              className="text-xs px-3 py-2 rounded-xl transition-colors font-medium flex items-center gap-1.5"
              style={{
                color: syncStatus === 'success' ? '#059669' : syncStatus === 'error' ? '#DC2626' : 'var(--text-secondary)',
                background: syncStatus === 'syncing' ? 'var(--bg-secondary)' : 'transparent',
              }}
              title={syncStatus === 'success' ? 'Sincronizzato!' : syncStatus === 'error' ? 'Errore di sync' : 'Sincronizza progresso'}>
              {syncStatus === 'syncing' ? (
