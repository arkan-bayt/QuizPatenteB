...
import { forceSyncToCloud, saveThemePreference, manualSync } from '@/logic/progressEngine';
      ...
      <div className="sticky top-0 z-30 glass-header">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 py-3.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {/* Sync button - TEMPORARY */}
            <button onClick={handleManualSync} disabled={isSyncing}
              className={`text-xs px-3 py-2 rounded-xl transition-colors font-medium flex items-center gap-1.5 ${
                syncStatus === 'done' ? 'text-emerald-500' :
                syncStatus === 'error' ? 'text-red-500' :
                'text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-secondary)]'
              }`}
              title={syncStatus === 'syncing' ? 'Sincronizzazione...' : syncStatus === 'done' ? 'Sincronizzato!' : syncStatus === 'error' ? 'Errore di sincronizzazione' : 'Sincronizza progressi'}>
              {isSyncing ? (
              ...
              <span className="hidden sm:inline">{syncStatus === 'syncing' ? '...' : syncStatus === 'done' ? '✓' : syncStatus === 'error' ? '!' : ''}</span>
            </button>
            {!isAppInstalled && (
