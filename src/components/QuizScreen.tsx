...
export default function QuizScreen() {
  ...
  if (!question) {
    // Quiz complete — show completion UI for assignment mode
    if (isComplete && isAssignmentMode) {
      const pctScore = total > 0 ? Math.round((correctCount / total) * 100) : 0;
        ...
        <div className="min-h-screen bg-mesh flex flex-col">
          ...
          <div className="glass-header px-5 py-3.5 sticky top-0 z-20">
            <div className="max-w-2xl mx-auto flex items-center justify-between">
              <button onClick={() => store.openStudentDashboard()}
              ...
              <div className="badge-modern text-[11px]" style={{
                background: 'rgba(79, 70, 229, 0.08)', borderColor: 'rgba(79, 70, 229, 0.12)', color: '#818CF8'
              }}>
          ...
          <div className="flex-1 flex flex-col items-center justify-center px-5 py-10">
            <div className="glass p-8 rounded-2xl text-center max-w-md w-full anim-up" style={{ boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.06)' }}>
              ...
              <h2 className="text-xl font-bold mb-2" style={{ color: 'var(--text-primary)' }}>
                {passed ? 'Complimenti!' : 'Quiz completato'}
              </h2>
              <p className="text-sm mb-6" style={{ color: 'var(--text-muted)' }}>
                {passed ? 'Hai superato il compito!' : 'Punteggio insufficiente. Puoi riprovare.'}
              </p>

              <div className="grid grid-cols-3 gap-3 mb-6">
                <div className="rounded-xl p-3 text-center" style={{ background: 'var(--bg-tertiary)' }}>
                  <p className="text-lg font-extrabold" style={{ color: '#059669' }}>{correctCount}</p>
                  <p className="text-[10px] font-semibold" style={{ color: 'var(--text-muted)' }}>Corrette</p>
                </div>
                <div className="rounded-xl p-3 text-center" style={{ background: 'var(--bg-tertiary)' }}>
                  <p className="text-lg font-extrabold" style={{ color: '#DC2626' }}>{wrongCount}</p>
                  <p className="text-[10px] font-semibold" style={{ color: 'var(--text-muted)' }}>Sbagliate</p>
                </div>
                <div className="rounded-xl p-3 text-center" style={{ background: 'var(--bg-tertiary)' }}>
                  <p className="text-lg font-extrabold" style={{ color: passed ? '#059669' : '#DC2626' }}>{pctScore}%</p>
                  <p className="text-[10px] font-semibold" style={{ color: 'var(--text-muted)' }}>Punteggio</p>
                </div>
              </div>

              {submittingResult ? (
                <div className="flex items-center justify-center gap-2 py-3">
                  <svg className="w-5 h-5 animate-spin" style={{ color: '#818CF8' }} fill="none" viewBox="0 0 24 24">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                  </svg>
                  <span className="text-sm font-semibold" style={{ color: 'var(--text-muted)' }}>Salvataggio risultato...</span>
                </div>
              ) : (
                <button onClick={() => store.openStudentDashboard()}
                  className="w-full py-3.5 text-white font-bold text-sm rounded-xl transition-colors"
    ...
    return <div className="min-h-screen bg-mesh flex items-center justify-center"><p style={{ color: 'var(--text-muted)' }}>Nessuna domanda</p></div>;
  }
